...
namespace Client
{
    public partial class UpdateDataPoint : Form
    {
        public Form2 Parent;
        public string ID;
        public UpdateDataPoint(Form2 ParentForm, string Data_ID)
        {
            Parent = ParentForm;
            ID = Data_ID;
            InitializeComponent();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (dateCheck(Date))
            {
                if (ageCheck(Age))
                {
                    ...
                    Parent.updateRow(sender, e, Int32.Parse(ID), Date, Country, Sex, Age);
